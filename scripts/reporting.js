...
async function exportParticipants(format) {
    try {
        ...
        enrichedData.sort((a, b) => {
            const timeA = new Date(a['Registration Time']);
            const timeB = new Date(b['Registration Time']);
            return timeB - timeA;
        });
...
async function exportTasks(format) {
    try {
        ...
        let roles = [];
        try {
            const rolesResponse = await fetch('../data/roles.json?v=' + new Date().getTime());
            if (rolesResponse.ok) {
                const rolesData = await rolesResponse.json();
                roles = rolesData.roles.map(r => r.id);
            }
        } catch (e) {
            console.warn('Could not load roles.json, using defaults');
            roles = ['chan', 'pauline', 'serena', 'sindy', 'may', 'herman', 'archie'];
        }
        ...
        for (const role of roles) {
            try {
                ...
                if (taskRows.length > 0) {
                    ...
                    // Add worksheet name
                    const sheetName = (data.name || role).substring(0, 31); // Limit sheet name length
...
async function exportSchedule(format) {
    try {
        // Define the schedule data structure matching the latest programme
        const scheduleData = [
            ['Time', 'Activity', 'Details'],
            ['8:30-9:00am', 'Registration & Seating', ''],
            ['9:00-9:40am', 'Opening Remarks & Thematic Framing', ''],
            ['9:40-10:10am', 'School Stories: Session 1', ''],
            ['10:10-10:50am', 'ACTIVITY 1: Structured Problem-Solving on Implementation Barriers', '10 mins: Generate Challenges\n2 mins: Pass to another table\n15 mins: Synthesizing for Sharing\n10 mins: Whole-Group Share-Back'],
            ['10:50-11:10am', 'Break + Poster time', ''],
            ['11:10am-11:40pm', 'School Stories: Session 2', ''],
            ['11:40-12:20pm', 'ACTIVITY 2: Community Resource Mapping', '10 mins: Community Brainstorming & Mapping\n10 mins: Analysis & Annotation\n3 mins: Synthesis & Sharing Preparation\n15 mins: Whole-Group Share-Back'],
            ['12:20-12:50pm', 'Closing Remarks & Reflection', '']
        ];
        ...
        if (format === 'xlsx') {
            ...
            scheduleData.forEach((row, index) => {
                if (row[2] && row[2].includes('\n')) {
                    rowHeights[index] = 15 * (row[2].split('\n').length + 1);
                }
            });
